...
class recsystools:
    def __init__(self,user_col,prod_col):
    ...
    def filter_triplets(self, data, min_user_count=5, min_item_count=5):

        # Only keep the triplets for items which were clicked on by at least min_sc users.

        if min_item_count > 0:
            itemcount = get_count(data, self.prod_col)
            data = data[data[self.prod_col].isin(itemcount.index[itemcount >= min_item_count])]
        ...
        if min_user_count > 0:
            usercount = get_count(data, self.user_col)
            data = data[data[self.user_col].isin(usercount.index[usercount >= min_user_count])]

        # Update both usercount and itemcount after filtering
        usercount, itemcount = get_count(data, self.user_col), get_count(data, self.prod_col) 
        return data, usercount, itemcount

    
    def split_train_test_proportion(self, data, test_prop=0.2):
        data_grouped_by_user = data.groupby(self.user_col)
